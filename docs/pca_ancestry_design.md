...
## Sample identifier contract

`IID` is the participant identifier used to join scientific outputs. `FID` is
retained as PLINK namespace/pedigree metadata and must never be assumed to be
`0`. Every PLINK `--keep` or `--remove` file generated by the workflow must
contain both `#FID` and `IID`, copied from the active PSAM. A one-column IID
file must not be used for PLINK sample selection because it can silently omit
participants with nonzero family IDs. Input PSAMs may omit FID; normalization
then supplies `FID=0` once at the input boundary.
